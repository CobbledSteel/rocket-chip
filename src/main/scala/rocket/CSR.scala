...
class CSRFile(
  ...
  io.vector.map { vio =>
    ...
    // FC+DroNet preemptible fix (V-race #2): a scalar `csrw vstart` this cycle (the
    // eager-V-save's `csrw vstart, x0`) must WIN over a concurrent VU set_vstart
    // write-back (a lingering ITERATIVE fault-check reporting a nonzero partial
    // vstart). Otherwise the VU override re-dirties reg_vstart after the csrw,
    // biasing the eager-save vse8.v element index + address. csrw is younger in
    // program order, so it wins.
    val scalar_vstart_wr = csr_wen && decoded_addr(CSRs.vstart)
    when (vio.set_vstart.valid && !scalar_vstart_wr) {
      set_vs_dirty := true.B
